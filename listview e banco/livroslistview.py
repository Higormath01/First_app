...
def main(page: ft.Page):
    ...
    def ver_detalhes(titulo, autor, descricao, categoria):
        txt.value = (f"Titulo: {titulo}; Autor: {autor}; Descrição: {descricao}; Categoria: {categoria}.")
    ...
    def gerencia_rota(e):
        ...
        page.views.append(
            View(
                "/",
                [
                    AppBar(title=Text("Cadastro de Livros"), bgcolor=Colors.BLUE_GREY_900),
                    input_nome,
                    input_descricao,
                    input_categoria,
                    input_autor,
                    ElevatedButton(text="Salvar Livro", on_click=salvar_livro, color=ft.CupertinoColors.SYSTEM_BLUE),
                    ElevatedButton(text="Exibir Lista", on_click=lambda _: page.go("/lista"), color=ft.CupertinoColors.SYSTEM_BLUE),
                ]
            )
        )
        if page.route == "/lista":
            ...
            page.views.append(
                View(
                    "/lista",
                    [
                        AppBar(title=Text("Lista de Livros"), bgcolor=Colors.BLUE_GREY_900),
                        lv_livro
                    ]
                )
            )
            ...
            page.views.append(
                View(
                    ...
                    [
                        AppBar(title=Text("Lista de Livros"), bgcolor=Colors.BLUE_GREY_900),
                        txt,
                        ElevatedButton(text="Voltar", on_click=lambda _: page.go("/lista"),
                                       color=ft.CupertinoColors.SYSTEM_BLUE)
                    ]
                )
            )
        page.update()
